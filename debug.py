...
for name in metadata[0]:
    file = 'ne_data/' + name + 'v3.npy'
    rulfiles = 'ne_data/' + name + '_rulv3.npy'
    feas = np.load(file, allow_pickle=True)
    ruls = np.load(rulfiles, allow_pickle=True)
    seqlen = ruls[0]
    print('fealen:', feas.shape, 'rullen:', ruls.shape, 'seqlen:', seqlen)
    print(feas[seqlen-1][0])