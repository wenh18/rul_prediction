...
def split_seq(fullseq, rul_labels, seqlen, seqnum):
    '''
    given a full sequence, this func splits it into fragments
    '''
...
def get_train_test_val(series_len=100, rul_factor=3000, seqnum=500, data_aug_scale_ratios=None):

    metadata = np.load('ne_data/fastcharge_meta.npy', allow_pickle=True)
    allseqs, allruls, batteryids = [], [], []
    batteryid = 0
    for batteryname in metadata:
        seqname = 'ne_data/' + batteryname + 'charge_v4.npy'
        lblname = 'ne_data/' + batteryname + 'charge_rulv4.npy'
...
if __name__ == '__main__':
    path1 = './ne_data/2019-01-24_batchdata_updated_struct_errorcorrect.mat'
    ...
    # np.save('fastcharge_meta.npy', cycle_train)
    # print(cycle_train)
    # exit(0)
    #     elif i in test_ind:
    ...
    def get_xy(cyc_num, seq_len=100, pkl_dir='ne_data/'):
        ...
        for i in cyc_num:
            ...
            print(extracted_fea.shape, all_lbl.shape)
            np.save(pkl_dir + key + 'charge_v4.npy', extracted_fea)
            np.save(pkl_dir + key + 'charge_rulv4.npy', all_lbl)
